...
//    [[self view] addGestureRecognizer:circle];
    ...
    mute.alpha = 1;
    mute.name = @"muteButton";//how the node is identified later0
    [self addChild:mute];
    
    pos.origin.x =  96;
    pos.origin.y = 95;
    SKShapeNode* controls = [SKShapeNode shapeNodeWithRect:pos];
    [controls setFillColor:[UIColor purpleColor]];
    controls.name = @"controlsButton";//how the node is identified later0
    controls.alpha = 1;
    ...
    beat.name = @"beatButton";//how the node is identified later0
    beat.alpha = 1;
    [self addChild:beat];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if([node.name isEqualToString:@"muteButton"])
    {
        g.ToggleMute();
        NSLog(@"mute");
    ...
    else if ([node.name isEqualToString:@"controlsButton"])
    {
        g.ToggleControls();
        NSLog(@"controls");
        [NSObject cancelPreviousPerformRequestsWithTarget:self];
    ...
    else if ([node.name isEqualToString:@"beatButton"])
    {
        g.ToggleBeat();
        NSLog(@"beat");
        [NSObject cancelPreviousPerformRequestsWithTarget:self];
